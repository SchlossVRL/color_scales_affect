...
p <- ggplot(ratings_trial_grouped, aes(x = hue, y = mean_rating, group = interaction(lightsat, shift), shape = factor(shift))) +
  geom_line(aes(colour = factor(lightness_numeric, levels=c(1,-1)),
              ...
              size = .5) +
  geom_point(aes(fill =light_sat_hue)) +
  facet_grid(shift ~ concept, scales = "free",) +  # Changed from facet_wrap to facet_grid
...
ggsave(paste0(plot_dir,'\exp2_facet_emotion_ratings.pdf'), dpi=300, height = 5, width=8,plot=p)
