...
```{r}
# List of packages
packages <- c("corrr", "ggplot2", "lme4", "lmerTest", "FactoMineR", "grid","png","cowplot","magick", "ggimage","dplyr","tidyverse","ggcorrplot","sjPlot","pheatmap","MuMIn")
...
ratings_trials_exp_image<- read_csv(paste0(data_dir,'/../ratings_trials_exp_2_image_statistics.csv')) ### contains average image color statistics

### scale the new predictors

ratings_trials_exp_image$image_L_scaled <- scale(ratings_trials_exp_image$image_L)
ratings_trials_exp_image$image_a_scaled <- scale(ratings_trials_exp_image$image_a)
ratings_trials_exp_image$image_b_scaled <- scale(ratings_trials_exp_image$image_b)
ratings_trials_exp_image$image_c_scaled <- scale(ratings_trials_exp_image$image_c)
...
# ratings_trials_exp_image%>%group_by(hue, saturation, lightness, shift)%>%slice(1) %>%ungroup()%>%group_by(hue, saturation, lightness)%>%summarize(image_a=mean(image_a), image_b = mean(image_b), image_c = mean(image_c), image_L=mean(image_L))

map_coords <- ratings_trials_exp_image%>%group_by(hue, saturation, lightness, shift)%>%slice(1)%>%ungroup()
map_coords<- map_coords[c('hue', 'saturation', 'lightness', 'shift','image_a','image_b','image_c','image_L')]
join_key <- c("hue", "saturation", "lightness", "shift")
exp2_pc_scores <- left_join(exp2_pc_scores, map_coords, by = join_key)

exp2_pc_scores$L_scaled <- scale(exp2_pc_scores$image_L)
exp2_pc_scores$a_scaled <- scale(exp2_pc_scores$image_a)
exp2_pc_scores$b_scaled <- scale(exp2_pc_scores$image_b)
exp2_pc_scores$C_scaled <- scale(exp2_pc_scores$image_c)


pc1_lm <- lm(PC1~ (a_scaled + b_scaled + L_scaled + C_scaled), data =exp2_pc_scores)
summary(pc1_lm)

pc2_lm <- lm(PC2~ (a_scaled + b_scaled + L_scaled + C_scaled), data =exp2_pc_scores)
summary(pc2_lm)
```
...
write.csv(ratings_trials_exp, paste0(data_dir,'/../ratings_trials_exp_2.csv'), row.names = FALSE)
```
...
### color space regression models

```{r}

positive_df <- ratings_trials_exp %>% filter(concept == 'positive')
negative_df <- ratings_trials_exp %>% filter(concept == 'negative')
startling_df <- ratings_trials_exp %>% filter(concept == 'startling')
calm_df <- ratings_trials_exp %>% filter(concept == 'calm')

positive_lmer<- lmer(response~ (a_scaled+b_scaled)*(L_scaled*C_scaled*shift_numeric)+(a_scaled+b_scaled+L_scaled+C_scaled||subject_id), data =positive_df,control=ctrl)
summary(positive_lmer)
cat(generate_lmer_latex_table(positive_lmer, model_name = "Positive"))

negative_lmer<- lmer(response~ (a_scaled+b_scaled)*(L_scaled*C_scaled*shift_numeric)+(a_scaled+b_scaled+L_scaled+C_scaled||subject_id), data =negative_df,control=ctrl)
summary(negative_lmer)
cat(generate_lmer_latex_table(negative_lmer, model_name = "Negative"))

startling_lmer<- lmer(response~ (a_scaled+b_scaled)*(L_scaled*C_scaled*shift_numeric)+(a_scaled+b_scaled+L_scaled+C_scaled||subject_id),data =startling_df,control=ctrl)
summary(startling_lmer)
cat(generate_lmer_latex_table(startling_lmer, model_name = "Startling"))

calm_lmer<- lmer(response~ (a_scaled+b_scaled)*(L_scaled*C_scaled*shift_numeric)+(a_scaled+b_scaled+L_scaled+C_scaled||subject_id),data =calm_df,control=ctrl)
summary(calm_lmer)
cat(generate_lmer_latex_table(calm_lmer, model_name = "Calm"))

```

### EXP 2 - Color Space Regression Models with map color coordinates

```{r}


positive_df <- ratings_trials_exp_image %>% filter(concept == 'positive')
negative_df <- ratings_trials_exp_image %>% filter(concept == 'negative')
startling_df <- ratings_trials_exp_image %>% filter(concept == 'startling')
calm_df <- ratings_trials_exp_image %>% filter(concept == 'calm')

positive_lmer_scales<- lmer(response~ (a_scaled+b_scaled)*(L_scaled*C_scaled)+(a_scaled+b_scaled+L_scaled+C_scaled||subject_id), data =positive_df,control=ctrl,REML = FALSE)
positive_lmer_images<- lmer(response~ (image_a_scaled+image_b_scaled)*(image_L_scaled*image_c_scaled)+(image_a_scaled+image_b_scaled+image_L_scaled+image_c_scaled||subject_id), data =positive_df,control=ctrl,REML = FALSE)


negative_lmer_scales<- lmer(response~ (a_scaled+b_scaled)*(L_scaled*C_scaled)+(a_scaled+b_scaled+L_scaled+C_scaled||subject_id), data =negative_df,control=ctrl,REML = FALSE)
negative_lmer_images<- lmer(response~ (image_a_scaled+image_b_scaled)*(image_L_scaled*image_c_scaled)+(image_a_scaled+image_b_scaled+image_L_scaled+image_c_scaled||subject_id), data =negative_df,control=ctrl,REML = FALSE)


startling_lmer_scales<- lmer(response~ (a_scaled+b_scaled)*(L_scaled*C_scaled)+(a_scaled+b_scaled+L_scaled+C_scaled||subject_id),data =startling_df,control=ctrl,REML = FALSE)
startling_lmer_images<- lmer(response~ (image_a_scaled+image_b_scaled)*(image_L_scaled*image_c_scaled)+(image_a_scaled+image_b_scaled+image_L_scaled+image_c_scaled||subject_id),data =startling_df,control=ctrl,REML = FALSE)


calm_lmer_scales<- lmer(response~ (a_scaled+b_scaled)*(L_scaled*C_scaled)+(a_scaled+b_scaled+L_scaled+C_scaled||subject_id),data =calm_df,control=ctrl,REML = FALSE)
calm_lmer_images<- lmer(response~ (image_a_scaled+image_b_scaled)*(image_L_scaled*image_c_scaled)+(image_a_scaled+image_b_scaled+image_L_scaled+image_c_scaled||subject_id),data =calm_df,control=ctrl,REML = FALSE)
...
```{r}
AIC(positive_lmer_scales, positive_lmer_images)
BIC(positive_lmer_scales, positive_lmer_images)
r.squaredGLMM(positive_lmer_scales)
r.squaredGLMM(positive_lmer_images)

AIC(negative_lmer_scales, negative_lmer_images)
BIC(negative_lmer_scales, negative_lmer_images)
r.squaredGLMM(negative_lmer_scales)
r.squaredGLMM(negative_lmer_images)

AIC(startling_lmer_scales, startling_lmer_images)
BIC(startling_lmer_scales, startling_lmer_images)
r.squaredGLMM(startling_lmer_scales)
r.squaredGLMM(startling_lmer_images)

AIC(calm_lmer_scales, calm_lmer_images)
BIC(calm_lmer_scales, calm_lmer_images)
r.squaredGLMM(calm_lmer_scales)
r.squaredGLMM(calm_lmer_images)

```

```{r}

cat(generate_lmer_latex_table(positive_lmer_scales, model_name = "Positive Color Scales"))
cat(generate_lmer_latex_table(negative_lmer_scales, model_name = "Negative Color Scales"))
cat(generate_lmer_latex_table(startling_lmer_scales, model_name = "Startling Color Scales"))
cat(generate_lmer_latex_table(calm_lmer_scales, model_name = "Calm Color Scales"))


cat(generate_lmer_latex_table(positive_lmer_images, model_name = "Positive Color Maps"))
cat(generate_lmer_latex_table(negative_lmer_images, model_name = "Negative Color Maps"))
cat(generate_lmer_latex_table(startling_lmer_images, model_name = "Startling Color Maps"))
cat(generate_lmer_latex_table(calm_lmer_images, model_name = "Calm Color Maps"))
```
