...
```{r}
# List of packages
packages <- c("corrr", "ggplot2", "lme4", "lmerTest", "FactoMineR", "grid","png","cowplot","magick", "ggimage","dplyr","tidyverse","ggcorrplot","jsonlite","simr")
```
...
data_dir <-paste0(analysis_dir,'/../data/exp_3_replication')
...
composite_scores$factor1 <- factor_scores$scores[,1]
composite_scores$factor2 <- factor_scores$scores[,2]
```
...
```{r}
m1_fa1<- lm(factor1 ~ color_condition_c*prompt_condition_c*climate_worry_c*affiliation_score_c, data = composite_scores)
summary(m1_fa1)

m1_fa2<- lm(factor2 ~ color_condition_c*prompt_condition_c*climate_worry_c*affiliation_score_c, data = composite_scores)
summary(m1_fa2)
```
...
power_sim <- powerSim(m1_factor, test = fixed("color_condition_c:affiliation_score_c", method="f"), nsim = 100)
print(power_sim)


extended_data <- extend(m1_factor1, along = "subject_id", n = 500)

power_sim_extended <- powerSim(extended_data, test = fixed("color_condition_c:climate_worry_c"), nsim = 1000)

# Print the power estimate
print(power_sim_extended)

sample_sizes<-numeric()
powers<-numeric()

for (i in seq(100, 800, by = 50)) {
extended_data <- extend(m1_factor2, along = "subject_id", n = i)
power_sim_extended <- powerSim(extended_data, test = fixed("color_condition_c:affiliation_score_c"), nsim = 1000)

sample_sizes <- c(sample_sizes, i)
powers <- c(powers, (power_sim_extended$x)/10)  # Save numeric power value
}
power_analysis_df<- data.frame(sample_sizes,powers)
power_analysis_df$sample_sizes<-as.numeric(power_analysis_df$sample_sizes)
power_analysis_df$powers<-as.numeric(power_analysis_df$powers)
ggplot(power_analysis_df, aes(x = sample_sizes, y = powers)) +
  geom_point()+
  geom_line() +
...
```{r}
library(simr)
pc2 <- powerCurve(extended_data, test = fixed("color_condition_c:affiliation_score_c"), sim=extended_data,along="subject_id", breaks = c(100,200,300,400,500,600,700))
